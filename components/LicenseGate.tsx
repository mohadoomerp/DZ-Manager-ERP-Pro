...
import { LicenseInfo, GeneratedLicense, NetworkRole } from '../types';
...
const LicenseGate: React.FC<LicenseGateProps> = ({ setLicense, licenseHistory = [] }) => {
  ...
  // FONCTION : Validation cryptographique du certificat
  const verifyCertificateSignature = async (certData: any) => {
    try {
      ...
      // Recalcul de la signature locale avec le sel secret
  ...
  const triggerActivation = async (inputKey: string, standaloneLic?: GeneratedLicense) => {
    ...
    setTimeout(() => {
      // Cas 1 : Activation via certificat signé (Standalone / Sans registre local)
      if (standaloneLic) {
        ...
        const newLicense: LicenseInfo = {
          ...
          modules: standaloneLic.modules, // Import modules from certificate
          maxUsers: standaloneLic.maxUsers // Import max users
        };
      ...
      // Cas 2 : Activation Manuelle via clé (Requiert le registre local)
      ...
      if (historyMatch || isUniversal) {
        const finalHubAddress = useManualConfig ? hubAddress : (networkRole === 'WORKSTATION' ? hubAddress : '127.0.0.1');
        const newLicense: LicenseInfo = {
          key: historyMatch?.key || 'DZ-PRO-2024-MANAGER',
          status: 'Active',
          expiryDate: historyMatch?.expiryDate || 'Permanente',
          owner: networkRole === 'HUB' ? 'Terminal Principal HUB' : 'Poste Client Réseau',
          networkRole: networkRole,
          hubAddress: finalHubAddress,
          databaseId: historyMatch?.databaseId || 'SQL_DZ_DEMO_SANDBOX',
          serverPort: serverPort,
          useManualConfig: useManualConfig,
          type: historyMatch?.type || 'BUSINESS',
          modules: historyMatch?.modules, // Use modules from history
          maxUsers: historyMatch?.maxUsers // Use maxUsers from history
        };
        localStorage.setItem('erp_license', JSON.stringify(newLicense));
        setLicense(newLicense);
      } else {
        setError("Clé invalide ou certificat non reconnu hors-ligne.");
      }
      setIsActivating(false);
    }, 1000);
  };
    ...
    reader.onload = async (event) => {
      try {
        ...
        if (isValid) {
           ...
           setError("Signature cryptographique invalide. Le certificat a été modifié ou corrompu.");
        }
      } catch (err) {
        setError("Erreur de lecture du certificat.");
      }
    };
      ...
      <div className="bg-white dark:bg-slate-900 w-full max-w-6xl rounded-[64px] shadow-2xl border border-slate-200 dark:border-slate-800 overflow-hidden flex flex-col md:flex-row min-h-[700px] animate-in zoom-in-95 duration-500">
        ...
        <div className="flex-1 p-12 bg-white dark:bg-slate-900 overflow-y-auto max-h-[85vh] custom-scrollbar">
           <div className="mb-12 flex items-center justify-between">
              <div>
                 <h3 className="text-2xl font-black text-slate-900 dark:text-white uppercase tracking-tighter">Portail d'Accès</h3>
                 <p className="text-[10px] text-slate-400 font-black uppercase tracking-[0.2em] mt-1">Validation cryptographique active</p>
              </div>
              <div className="flex space-x-2">
                ...
                <button 
                  ...
                  className="px-6 py-2.5 bg-emerald-600 text-white rounded-2xl text-[10px] font-black uppercase flex items-center shadow-lg hover:bg-emerald-700 transition-all active:scale-95"
                >
                   <FileUp size={14} className="mr-2" /> Importer Certificat (.dzlic)
                </button>
              </div>
           </div>
           ...
           <form onSubmit={(e) => { e.preventDefault(); triggerActivation(key); }} className="space-y-10">
                 ...
                 <div className="col-span-full space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-6">
                       <button type="button" onClick={() => setNetworkRole('HUB')} className={`p-6 rounded-[32px] border-4 transition-all text-left ${networkRole === 'HUB' ? 'bg-indigo-50 border-indigo-600' : 'bg-slate-50 border-transparent opacity-50'}`}>
                          <Server size={24} className={networkRole === 'HUB' ? 'text-indigo-600' : 'text-slate-400'} />
                          <p className="mt-4 text-xs font-black uppercase text-slate-900">Serveur Hub (Principal)</p>
                       </button>
                       <button type="button" onClick={() => setNetworkRole('WORKSTATION')} className={`p-6 rounded-[32px] border-4 transition-all text-left ${networkRole === 'WORKSTATION' ? 'bg-emerald-50 border-emerald-600' : 'bg-slate-50 border-transparent opacity-50'}`}>
                          <Laptop size={24} className={networkRole === 'WORKSTATION' ? 'text-emerald-600' : 'text-slate-400'} />
                          <p className="mt-4 text-xs font-black uppercase text-slate-900">Poste Client (Réseau)</p>
                       </button>
                    </div>
                 </div>
              ...
              {error && (
                <div className="p-6 bg-rose-50 border-2 border-rose-100 rounded-[32px] flex items-center space-x-4 animate-bounce">
                   <ShieldAlert size={24} className="text-rose-600" />
                   <p className="text-rose-600 text-xs font-black uppercase tracking-widest leading-relaxed">{error}</p>
                </div>
              )}

              <button type="submit" disabled={isActivating} className="w-full bg-slate-900 dark:bg-indigo-600 hover:bg-indigo-700 text-white py-6 rounded-[32px] font-black uppercase text-xs tracking-[0.3em] shadow-2xl transition-all flex items-center justify-center">
                 {isActivating ? <RefreshCw size={24} className="animate-spin" /> : <span>Activer via Clé</span>}
              </button>
           </form>
           
           <div className="mt-8 p-8 bg-indigo-50/50 rounded-3xl flex items-start space-x-4 border border-indigo-100">
              ...
              <div>
                <p className="text-[10px] text-indigo-700 font-bold uppercase leading-relaxed">
                  Note technique : L'importation du certificat (.dzlic) utilise le moteur SHA-256 pour valider l'authenticité de votre licence même sans historique local.
                </p>
              </div>
           </div>
        </div>
