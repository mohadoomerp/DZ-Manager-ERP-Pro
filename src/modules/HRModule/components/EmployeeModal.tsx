...
const EmployeeModal: React.FC<EmployeeModalProps> = ({ isOpen, onClose, editingEmp, onSave, companySettings }) => {
  ...
  // Available Rubrics: Prefer custom config, fallback to common
  const availableRubriques = useMemo(() => {
      return (companySettings.customRubriques && companySettings.customRubriques.length > 0) 
          ? companySettings.customRubriques 
          : COMMON_RUBRIQUES;
  }, [companySettings]);
  ...
  useEffect(() => {
    if (isOpen) {
        if (editingEmp) {
            ...
            // Default rubrics from available list if they match typical defaults
            const defaults = availableRubriques.filter(r => ['PANIER', 'TRANSPORT', 'IEP'].includes(r.id)).map(r => ({...r, value: 0}));
            setActiveRubriques(defaults);
            setActiveLoans([]);
        }
        setModalTab('info');
    }
  }, [isOpen, editingEmp, availableRubriques]);
  ...
  const handleAddRubrique = () => {
      ...
      const template = availableRubriques.find(r => r.id === selectedRubricId);
      if (template) {
                  ...
                  <div className="pt-4 border-t border-white/10">
                      ...
                      <div className="flex gap-2 mb-4">
                          ...
                          >
                              ...
                              {availableRubriques.filter(r => !activeRubriques.find(ar => ar.id === r.id)).map(r => (
