...
interface SuppliersModuleProps {
  ...
  onDeleteSupplier: (id: string) => void; // Nouvelle prop
}

const SuppliersModule: React.FC<SuppliersModuleProps> = ({ suppliers, setSuppliers, currentUser, onAddLog, onDeleteSupplier }) => {
                ...
                <tr key={s.id} className="hover:bg-slate-50/50 transition-colors group">
                  ...
                  <td className="px-8 py-5 text-right">
                    <div className="flex items-center justify-end space-x-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      ...
                      <button onClick={() => onDeleteSupplier(s.id)} className="p-2.5 bg-white border border-slate-200 text-rose-500 hover:bg-rose-500 hover:text-white rounded-xl shadow-sm transition-all"><Trash2 size={16} /></button>
                    </div>
                  </td>
                </tr>
              ))}
