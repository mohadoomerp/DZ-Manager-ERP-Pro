...
interface InventoryModuleProps {
  ...
  onDeleteProduct: (id: string) => void; // Nouvelle prop
}
...
const InventoryModule: React.FC<InventoryModuleProps> = ({ products, setProducts, currentUser, onAddLog, onDeleteProduct }) => {
  ...
  const stats = useMemo(() => {
    return {
      totalValue: activeProducts.reduce((acc, p) => acc + (p.stock * p.priceRetail), 0),
      alerts: activeProducts.filter(p => p.isPhysical && p.stock <= p.minStock).length,
      maintenance: activeProducts.filter(p => p.status === 'Maintenance').length,
  ...
  }, [activeProducts, searchTerm, filterStatus]);
                ...
                <tr key={p.id} className="hover:bg-slate-50 transition-colors group">
                  ...
                  <td className="px-8 py-5 text-right">
                    <div className="flex items-center justify-end space-x-2 opacity-0 group-hover:opacity-100 transition-opacity">
                       ...
                       <button onClick={() => onDeleteProduct(p.id)} className="p-2 text-rose-600 hover:bg-rose-50 rounded-xl"><Trash2 size={16} /></button>
                    </div>
                  </td>
                </tr>
              ))}
