...
interface EventsModuleProps {
  ...
  onDeleteEvent: (id: string) => void; // Nouvelle prop
}
...
const TABS = [
  ...
  { id: 'affectation', label: 'Liste Stands', icon: <Grid3X3 size={16} /> },
  { id: 'visual_plan', label: 'Plan & Chapiteaux', icon: <Map size={16} /> },
  { id: 'staff', label: 'Staff', icon: <Briefcase size={16} /> },
  { id: 'expenses', label: 'Dépenses', icon: <Wallet size={16} /> },
  { id: 'billing', label: 'Facturation', icon: <Receipt size={16} /> }
];

const EventsModule: React.FC<EventsModuleProps> = ({ 
  clients, products, events = [], setEvents, onStockUpdate, onGenerateInvoice, onNavigate, currentUser, users, invoices, companySettings, onTransactionAdd, onAddLog, onDeleteEvent
}) => {
  const [view, setView] = useState<'list' | 'create' | 'manage'>('list');
  const [activeTab, setActiveTab] = useState<'dashboard' | 'participants' | 'affectation' | 'visual_plan' | 'staff' | 'billing' | 'expenses'>('dashboard');
  ...
  const visibleTabs = useMemo(() => {
    if (currentUser.role === 'SALES') {
        return TABS.filter(tab => tab.id !== 'staff');
    }
    return TABS;
  }, [currentUser.role]);
  ...
  const handleCreateOrUpdateEvent = (formData: any) => {
    ...
    // Added missing visitors and exhibitorTeams properties to match EventBooking interface
  ...
  if (view === 'manage' && activeEvent) {
    return (
      <div className="bg-white dark:bg-slate-900 rounded-[48px] shadow-2xl border border-slate-200 dark:border-slate-800 overflow-hidden animate-in slide-in-from-bottom-8 duration-500 flex flex-col h-[85vh]">
        ...
        {/* Content */}
        <div className="flex-1 overflow-y-auto p-4 md:p-10 custom-scrollbar bg-slate-50 dark:bg-slate-800/20 no-print">
