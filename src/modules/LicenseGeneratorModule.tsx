...
// MAPPING VISUEL DES OFFRES (Data from constants)
const OFFERS_UI = [
  { ...LICENSE_PLANS['STA'], id: 'STARTUP', color: 'indigo', icon: <Rocket size={24} />, price: '25,000 DA/an', desc: 'Caisse & Vente simple. Idéal pour débuter.' },
  { ...LICENSE_PLANS['RET'], id: 'RETAIL', color: 'blue', icon: <ShoppingBag size={24} />, price: '45,000 DA/an', desc: 'Gestion complète Achat/Vente/Stock pour grossistes.' },
  { ...LICENSE_PLANS['SER'], id: 'SERVICE', color: 'cyan', icon: <Briefcase size={24} />, price: '50,000 DA/an', desc: 'Suivi prospects, devis et facturation de services.' },
  { ...LICENSE_PLANS['HR_'], id: 'HR_FOCUS', color: 'rose', icon: <Users size={24} />, price: '40,000 DA/an', desc: 'Gestion du personnel, pointage biométrique et paie.' },
  { ...LICENSE_PLANS['EVE'], id: 'EVENTS', color: 'purple', icon: <CalendarDays size={24} />, price: '60,000 DA/an', desc: 'Organisation de salons, foires et locations.' },
  { ...LICENSE_PLANS['BUS'], id: 'BUSINESS', color: 'emerald', icon: <Award size={24} />, price: '85,000 DA/an', desc: 'ERP complet PME : Compta, RH, Commercial, Stock.' },
  { ...LICENSE_PLANS['COR'], id: 'CORPORATE', color: 'amber', icon: <Building size={24} />, price: 'Sur Devis', desc: 'Solution illimitée multi-sites avec toutes les options.' }
];

const LicenseGeneratorModule: React.FC<LicenseGeneratorModuleProps> = ({ history, setHistory }) => {
  const [owner, setOwner] = useState('');
  const [authorizedEmail, setAuthorizedEmail] = useState('');
  const [selectedOffer, setSelectedOffer] = useState(OFFERS_UI[5]); // Default to Business
  ...
  const generateNewKey = () => {
    ...
    setTimeout(() => {
      ...
      // Trouver le code préfixe (ex: 'RET' pour RETAIL)
      const typeCode = Object.keys(LICENSE_PLANS).find(key => LICENSE_PLANS[key].type === selectedOffer.type) || 'BUS';
      ...
      const newEntry: GeneratedLicense = {
        ...
        maxUsers: selectedOffer.maxUsers,
                            ...
                            <div>
                                ...
                                <div className="space-y-1">
                                    <div className="flex items-center text-[8px] font-bold text-slate-300 uppercase">
                                        <Check size={10} className="mr-1 text-emerald-400" /> {offer.maxUsers === 999 ? 'Utilisateurs Illimités' : `${offer.maxUsers} Utilisateurs`}
                                    </div>
                                    <div className="flex items-center text-[8px] font-bold text-slate-300 uppercase">
             ...
             {generatedKey && (
                <div className="p-6 bg-indigo-500/10 border border-indigo-500/20 rounded-3xl w-full flex flex-col items-center space-y-3">
                   ...
                   <div className="flex justify-between w-full text-[9px] font-bold text-slate-400 uppercase px-4">
                      ...
                      <span>Users : {selectedOffer.maxUsers}</span>
                   </div>
