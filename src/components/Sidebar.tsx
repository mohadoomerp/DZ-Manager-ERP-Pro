...
const Sidebar: React.FC<SidebarProps> = ({ 
  ...
  const sidebarClasses = `
    fixed inset-y-0 z-[100] transform transition-transform duration-300 ease-in-out border-slate-800 flex flex-col no-print
    ${theme === 'midnight' ? 'bg-slate-900 border-slate-800' : 'bg-slate-900 border-slate-800'}
    ${isRtl ? 'right-0 border-l' : 'left-0 border-r'}
    ${isMobileOpen ? 'translate-x-0 w-[85vw] max-w-[300px] shadow-2xl' : (isRtl ? 'translate-x-full' : '-translate-x-full') + ' w-72'}
  ...
  return (
    <>
      {/* Mobile Backdrop */}
      <div 
        className={`fixed inset-0 bg-slate-950/60 backdrop-blur-sm z-[90] md:hidden transition-opacity duration-300 ${isMobileOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}
        onClick={onCloseMobile}
      />

      <aside className={sidebarClasses}>
        <div className="h-20 md:h-24 flex items-center px-6 border-b border-slate-800 shrink-0 relative justify-between md:justify-start">
          <div 
          ...
          {(isExpanded || isMobileOpen) && (
            <div className="mx-4 overflow-hidden animate-in fade-in slide-in-from-left-2 duration-300 flex-1">
          ...
          <button onClick={onCloseMobile} className={`md:hidden p-2 text-slate-400 hover:text-white bg-slate-800/50 rounded-xl`}><X size={20} /></button>
        </div>

        <nav className="flex-1 overflow-y-auto py-4 md:py-6 px-3 space-y-1 custom-scrollbar">
          {filteredItems.map((item) => {
            ...
            return (
              <button
                key={item.id}
                onClick={() => { onModuleChange(item.id); onCloseMobile(); }}
