...
import { ModuleType, Partner, Product, Employee, Invoice, Transaction, Purchase, EventBooking, AttendanceRecord, User, LicenseInfo, SessionInfo, CompanySettings, Lead, GeneratedLicense, AuditLog, DailyClosing, Candidate, TimeClockLog } from './types';
import { MENU_ITEMS, ROLE_PERMISSIONS } from './constants';
...
// FIX: Corrected the import path for SalesModule to point to the implementation in src/modules/sales
...
import { ChevronLeft, Sparkles, RefreshCw, LogOut, Server, Laptop, Languages, Palette, CloudLightning, Menu as MenuIcon, Usb, Save, Wifi, WifiOff, DatabaseZap, AlertTriangle } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    if (license?.networkRole === 'HUB') {
      idb.get('mirror_dir_handle').then(handle => {
        if (handle) setDirHandle(handle);
      }).catch(e => console.warn("IDB Access Error", e));
    }
  }, [license]);
  ...
  // Unified logger for Mission Control
  const addAuditLog = useCallback((action: AuditLog['action'], module: ModuleType, details: string) => {
    ...
    setAuditLogs(prev => [newLog, ...prev].slice(0, 1000)); // Garder les 1000 derniers logs
  }, [currentUser]);
    ...
    if (data.type === 'DATA_SYNC' || data.type === 'FULL_PUSH') {
      ...
      if (license?.networkRole === 'HUB' && data.type === 'DATA_SYNC') {
        ...
        connectionsRef.current.forEach(c => {
          if (c.open && c.peer !== conn.peer && c.peer !== senderId) {
            c.send(data);
          }
        });
      }
  ...
  const connectToHub = useCallback(() => {
    ...
    if (workstationConnRef.current) {
        try { workstationConnRef.current.close(); } catch (e) {}
        workstationConnRef.current = null;
    }

    const hubId = `dz-erp-hub-${license.databaseId}`;
    
    const conn = peerRef.current.connect(hubId, {
        reliable: true,
        metadata: { userId: currentUser.id, userName: currentUser.name }
    });
    
    workstationConnRef.current = conn;

    (conn as any).on('open', () => {
        console.info("Liaison établie avec le Hub.");
    ...
    (conn as any).on('error', (err: any) => {
        console.error("Erreur de liaison Hub:", err);
        setNetworkStatus('disconnected');
        workstationConnRef.current = null;
        if (retryTimeoutRef.current) clearTimeout(retryTimeoutRef.current);
        retryTimeoutRef.current = setTimeout(connectToHub, 5000);
    });
  }, [license, handleIncomingData, currentUser.id, currentUser.name]);
  ...
  const initNetwork = useCallback(() => {
    ...
    setTimeout(() => {
        const peerId = license.networkRole === 'HUB'
          ? `dz-erp-hub-${license.databaseId}`
          : `dz-erp-station-${currentUser.id}-${Math.random().toString(36).substr(2, 5)}`;
        
        myPeerId.current = peerId;

        console.log(`Initialisation PeerJS (Role: ${license.networkRole}, ID: ${peerId})`);
        ...
        (peer as any).on('open', (id: string) => {
          ...
          console.info(`Moteur réseau activé (ID: ${id})`);
          if (license.networkRole === 'HUB') {
            setNetworkStatus('connected');
          } else {
            connectToHub();
        ...
        (peer as any).on('connection', (conn: DataConnection) => {
          (conn as any).on('open', () => {
            if (license.networkRole === 'HUB') {
              ...
              setActiveSessions(prev => {
                 const filtered = prev.filter(s => s.sessionId !== conn.peer);
                 return [...filtered, {
                    sessionId: conn.peer,
                    userName: metadata?.userName || 'Utilisateur',
                    deviceName: 'Poste Client',
                    lastActive: new Date().toISOString(),
                    dbId: license.databaseId,
                    status: 'online'
                 }];
              });
              ...
              setTimeout(() => {
                 if (conn.open) {
                    conn.send({ type: 'FULL_PUSH', payload: allDataForSync.current, sourcePeerId: myPeerId.current });
                 }
              }, 500);
            }
          });
          ...
          (conn as any).on('close', () => {
            if (license.networkRole === 'HUB') {
              ...
              setActiveSessions(prev => prev.filter(s => s.sessionId !== conn.peer));
            }
          });
        });
        ...
        (peer as any).on('error', (err: any) => {
          ...
          isInitializing.current = false; // Reset init flag on error
          
          if (err.type === 'unavailable-id') {
              console.warn("ID indisponible. Tentative de reconnexion...");
              ...
              retryTimeoutRef.current = setTimeout(() => {
                  ...
                  initNetwork();
              }, 2000 + Math.random() * 1000); // Backoff + Jitter to resolve conflict
          } 
          else if (err.type === 'peer-unavailable' && license.networkRole === 'WORKSTATION') {
              setNetworkStatus('disconnected');
          }
        ...
        // Also checking for peer.on('close') if present
  ...
  }, [license, currentUser.id, handleIncomingData, connectToHub]);
  ...
  }, [license?.databaseId, license?.networkRole]);

  const forceNetworkReset = () => {
      isInitializing.current = false;
      if (peerRef.current) {
          peerRef.current.destroy();
          peerRef.current = null;
  ...
  const broadcastData = useCallback((payload: any) => {
    ...
    const message = {
      ...
      sourcePeerId: myPeerId.current,
      timestamp: Date.now()
    };

    if (license.networkRole === 'HUB') {
      connectionsRef.current.forEach(c => { if (c.open) c.send(message); });
    } 
    else if (license.networkRole === 'WORKSTATION' && workstationConnRef.current?.open) {
      workstationConnRef.current.send(message);
    }
  ...
  useEffect(() => {
    ...
    save('closings', closings); // Fixed: Save closings
    ...
    const allData = { clients, suppliers, products, invoices, transactions, closings, purchases, events, employees, candidates, attendance, timeClockLogs, companySettings, users, leads, auditLogs };
    
    if (!isIncomingUpdate.current) {
        if (broadcastTimeoutRef.current) clearTimeout(broadcastTimeoutRef.current);
        broadcastTimeoutRef.current = setTimeout(() => {
            broadcastData(allData);
        }, 2000);
    }
  ...
  }, [clients, suppliers, products, invoices, transactions, closings, purchases, events, employees, candidates, attendance, timeClockLogs, users, companySettings, leads, auditLogs, license, broadcastData, dirHandle]);
  ...
  const renderContent = () => {
    // Vérification de sécurité : si le module demandé n'est pas dans la licence, redirection Dashboard
    if (activeModule !== ModuleType.DASHBOARD && license?.modules && !license.modules.includes(activeModule)) {
       // Si même le dashboard n'est pas autorisé (cas rare), on ne rend rien ou une erreur
       if (!license.modules.includes(ModuleType.DASHBOARD)) return <div className="p-8 text-center text-slate-500 font-black">Accès restreint par la licence.</div>;
    ...
    switch (activeModule) {
      ...
      case ModuleType.SALES: return <SalesModule clients={clients} products={products} invoices={invoices} companySettings={companySettings} onSaveInvoice={(inv) => { setInvoices(prev => [{...inv, createdBy: currentUser.name, updatedAt: Date.now()}, ...prev]); addAuditLog('CREATE', ModuleType.SALES, `Nouveau document ${inv.type} N°${inv.number}`); }} onUpdateInvoice={(upd) => { setInvoices(prev => prev.map(i => i.id === upd.id ? {...upd, updatedAt: Date.now()} : i)); addAuditLog('UPDATE', ModuleType.SALES, `Mise à jour document N°${upd.number} (${upd.status})`); }} onDeleteInvoice={(id) => { const inv = invoices.find(i => i.id === id); setInvoices(prev => prev.filter(i => i.id !== id)); addAuditLog('DELETE', ModuleType.SALES, `Suppression facture ${inv?.number}`); }} onStockUpdate={(pId, ch) => setProducts(prev => prev.map(p => p.id === pId ? {...p, stock: Math.max(0, p.stock + ch), updatedAt: Date.now()} : p))} onTransactionAdd={(tx) => { setTransactions(prev => [{...tx, updatedAt: Date.now()}, ...prev]); addAuditLog('CREATE', ModuleType.FINANCE, `Encaissement sur facture ${tx.title}`); }} />;
      case ModuleType.CLIENTS: return <ClientsModule clients={clients} setClients={setClients} currentUser={currentUser} events={events} invoices={invoices} onAddLog={addAuditLog} />;
      case ModuleType.SUPPLIERS: return <SuppliersModule suppliers={suppliers} setSuppliers={setSuppliers} currentUser={currentUser} onAddLog={addAuditLog} />;
      case ModuleType.EVENTS: return <EventsModule clients={clients} products={products} events={events} setEvents={setEvents} onStockUpdate={(pId, ch) => setProducts(prev => prev.map(p => p.id === pId ? {...p, stock: p.stock + ch, updatedAt: Date.now()} : p))} onGenerateInvoice={(inv) => { setInvoices(prev => [{...inv, updatedAt: Date.now()}, ...prev]); addAuditLog('CREATE', ModuleType.EVENTS, `Facture générée pour dossier événementiel`); }} onNavigate={setActiveModule} currentUser={currentUser} users={users} invoices={invoices} companySettings={companySettings} onTransactionAdd={(tx) => { setTransactions(prev => [{...tx, updatedAt: Date.now()}, ...prev]); addAuditLog('CREATE', ModuleType.FINANCE, `Dépense événementielle : ${tx.title}`); }} onAddLog={addAuditLog} />;
      case ModuleType.INVENTORY: return <InventoryModule products={products} setProducts={setProducts} currentUser={currentUser} onAddLog={addAuditLog} />;
      case ModuleType.PURCHASES: return <PurchasesModule suppliers={suppliers} products={products} purchases={purchases} setPurchases={setPurchases} setProducts={setProducts} setTransactions={setTransactions} currentUser={currentUser} onAddLog={addAuditLog} />;
      case ModuleType.FINANCE: return <FinanceModule transactions={transactions} setTransactions={setTransactions} closings={closings} setClosings={setClosings} currentUser={currentUser} onAddLog={addAuditLog} onDeleteTransaction={(id) => { const tx = transactions.find(t => t.id === id); if(tx && confirm('Supprimer cette transaction ?')) { setTransactions(prev => prev.filter(t => t.id !== id)); addAuditLog('DELETE', ModuleType.FINANCE, `Suppression transaction ${tx.title}`); } }} />;
      case ModuleType.HR: return <HRModule employees={employees} setEmployees={setEmployees} candidates={candidates} setCandidates={setCandidates} attendance={attendance} setAttendance={setAttendance} onAddLog={addAuditLog} companySettings={companySettings} />;
  ...
  return (
    <div className={`flex h-screen ${theme === 'midnight' ? 'bg-slate-950 text-slate-200' : 'bg-slate-50'} overflow-hidden font-sans`}>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className={`h-16 ${theme === 'midnight' ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'} border-b flex items-center justify-between px-6 z-[60] shadow-sm`}>
