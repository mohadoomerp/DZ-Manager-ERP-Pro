...
const PurchasesModule: React.FC<PurchasesModuleProps> = ({ suppliers, products, purchases, setPurchases, setProducts, setTransactions, currentUser, onAddLog }) => {
  ...
  const deletePurchase = (id: string) => {
    ...
    if(p) {
        setPurchases(purchases.filter(pur => pur.id !== id).map(x => ({...x, updatedAt: Date.now()})));
        onAddLog('DELETE', ModuleType.PURCHASES, `Suppression achat ${p.number}`);
    }
  }
