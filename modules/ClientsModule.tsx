...
const ClientsModule: React.FC<ClientsModuleProps> = ({ clients, setClients, currentUser, events, invoices, onAddLog }) => {
  ...
  const filteredClients = useMemo(() => {
    ...
    if (currentUser.role === 'SALES') {
      ...
      baseClients = clients.filter(client => clientsInAssignedEvents.has(client.id) || !allClientsInAnyEvent.has(client.id));
    }
  ...
  }, [clients, searchTerm, currentUser, events, filterEvent]);
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
            ...
            <div key={client.id} className="bg-white dark:bg-slate-900 p-8 rounded-[40px] border border-slate-200 dark:border-slate-800 shadow-sm hover:shadow-2xl transition-all group">
              <div className="flex justify-between items-start mb-6">
                ...
                <div className="flex space-x-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  ...
                  <button onClick={() => { if(confirm('Supprimer ?')) { setClients(clients.filter(c => c.id !== client.id)); onAddLog('DELETE', ModuleType.CLIENTS, `Suppression client : ${client.name}`); } }} className="p-3 bg-slate-50 dark:bg-slate-800 text-slate-500 hover:text-rose-600 rounded-xl"><Trash2 size={16} /></button>
                </div>
              </div>
                 ...
                 {filteredClients.map(client => (
                   <tr key={client.id} className="hover:bg-slate-50 transition-colors group">
                      ...
                      <td className="px-8 py-5 text-right">
                         ...
                         <button onClick={() => { if(confirm('Supprimer ?')) { setClients(clients.filter(c => c.id !== client.id)); onAddLog('DELETE', ModuleType.CLIENTS, `Suppression client : ${client.name}`); } }} className="p-2.5 bg-rose-50 text-rose-600 rounded-xl"><Trash2 size={16} /></button>
                      </td>
                   </tr>
                 ))}
              </tbody>
      ...
      {isScannerOpen && <OCRScanner targetType="client" onClose={() => setIsScannerOpen(false)} onScanComplete={(data) => { setScanData(data); setIsModalOpen(true); }} />}
